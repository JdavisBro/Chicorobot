...
class DogCog(commands.Cog):
    ...
    async def make_dog(
        ...
        if not animated:
            ...
            print( f"{imagemagick} -delay 1x8 -loop 0 -dispose Background {addcrop}{temp / '*.png'} {temp / 'out.gif'}")
            process = await asyncio.create_subprocess_shell(
                f"{imagemagick} -delay 1x8 -loop 0 -dispose Background {addcrop}{temp / '*.png'} {temp / 'out.gif'}",
    ...
    async def random_dog(self, interaction: discord.Interaction, use_in_game_colors: bool=True, use_palette: str="None", add_hat2: bool=False, animated: bool=False, random_animation: bool=False):
        ...
        await self.make_dog(
            ...
            coltwo,
            colthree,
            extra_text=f"\nPalette: {chosen}" + (" - Gay Mode Active." if active else ""),
            view=view
        )
